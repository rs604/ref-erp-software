-- ============================================================
-- 38 EVERY ROW LOOKED EDITED
--
-- The edit check compared the stored value against the incoming one AS
-- TEXT. A quantity of 10 is stored as "10.0000" and arrives as "10" --
-- the same number, different text. So every unchanged quantity, rate and
-- amount counted as an edit: three false entries per row, about 68,000
-- of them on the first re-sync, burying the handful that were real.
--
...
begin
  ...
  -- 1. Record real edits BEFORE overwriting, so the old value survives.
  --    Compared as NUMBERS: 10.0000 and 10 are the same quantity.
