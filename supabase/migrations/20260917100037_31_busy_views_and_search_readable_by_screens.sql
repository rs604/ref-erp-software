-- ============================================================
-- 31 THE SCREENS COULD NOT READ ANYTHING. FIXED.
--
-- Migration 27 revoked everything on busy_history so that no screen
-- could ever combine the two firms. That part was right. But the two
-- company views and busy_search() were all security_invoker, which
-- means they read the base table AS THE PERSON CALLING THEM -- and that
-- person has no rights on it. So every one of them failed with
-- "permission denied for table busy_history".
--
-- The price history screen would have been dead on arrival. Found by
-- calling the function as a signed-in user instead of as the owner.
--
-- The fix keeps BOTH guarantees, and moves the permission test into the
-- view itself rather than relying on the base table's RLS:
--   * busy_history stays unreadable, so nothing can select across firms
--   * each view reads the base table as its owner, but carries its own
--     "do you hold busy_data.view" test in the WHERE clause
--   * busy_search runs as definer, checks the permission, and will only
--     ever accept ONE company per call
-- ============================================================

drop view if exists public.busy_history_ref;
drop view if exists public.busy_history_rs;

-- security_invoker is deliberately OFF here so the view can reach the
-- base table, and the permission test is written into the view instead.
...
comment on view public.busy_history_ref is
  'Raghbir Erectors & Fabricators only, and only for someone holding busy_data.view. A screen under the REF menu reads this and can see nothing else. The base table stays unreadable, so no screen can combine the two firms.';
comment on view public.busy_history_rs is
  'RS Industries only, and only for someone holding busy_data.view. Two legally separate firms -- no report may ever combine them, and no screen could.';

grant select on public.busy_history_ref to authenticated;
grant select on public.busy_history_rs  to authenticated;

-- The rate statistics span both firms by design (they are grouped by
-- company), so they stay internal to the search function.
revoke all on public.busy_item_rate_stats from authenticated, anon;

-- ---------- The search runs as definer, and checks for itself ----------
...
comment on function public.busy_search(text,text,text,text,date,date,int,int) is
  'The one way a screen reads Busy history. Checks the permission itself, insists on ONE named firm per call, then searches: exact first, then the same words ignoring spacing and punctuation in any order, then fuzzy for typos. Fuzzy is only reached when NO number was typed -- a size must match exactly, because 80X40X2.5 and 80X40X3 are different pipes.';
