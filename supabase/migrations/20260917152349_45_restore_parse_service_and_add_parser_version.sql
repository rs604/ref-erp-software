-- ============================================================
-- 45 MDBTOOLS STAYS, AND EVERY BATCH RECORDS ITS PARSER
--
-- Migration 44 said the parse service was not needed, because a pure
-- Python reader had read a .bds file. That was tested on ONE file.
-- Tested on all 24 it reads 9 and FAILS ON 15 -- every year before 2024,
-- with "Could not find overflow record data page overflow pointer".
-- Older Access files spread a record across overflow pages and the
-- library cannot follow them. mdbtools reads all 24.
--
-- So parse_service_url is a live setting again and the container is back.
--
-- AND the thing that makes this kind of mistake visible next time:
-- every batch now records WHICH PARSER produced its rows. If a row ever
-- looks wrong, you can tell what read it. A batch carrying two different
-- parser versions is refused outright -- that means the file was built
-- from two different runs, which is exactly the version skew that made
-- keeping the parser out of the uploader the right call.
-- ============================================================

comment on column public.busy_sync_settings.parse_service_url is
  'Where the parse service runs. It has to be a container with mdbtools installed: a Supabase edge function cannot read a Busy file, and the pure-Python readers fail on every year before 2024. See busy/parse-service/README.md.';

alter table public.busy_import_batches
  add column parser_version text;

comment on column public.busy_import_batches.parser_version is
  'Which parser read this file, taken from the rows themselves. If a row ever looks wrong you can tell what produced it. Two different versions inside one batch means the file was assembled from two runs, and the import refuses it.';

create index busy_import_batches_parser_idx on public.busy_import_batches (parser_version);
