...
Outputs TWO row kinds:
  ...
  kind='ledger'  ledger postings from payments and journals  (added 17 Sep 2026)

The ledger rows are what make "GST actually paid" possible. The item rows are
unchanged from the version proven against 24 files / 22,732 rows.
"""
import subprocess, csv, io, re, sys

VCH = {'2':'Purchase Bill','9':'Sales Invoice','11':'Delivery Challan',
       '12':'Purchase Order','13':'Sales Order',
       '16':'Journal','19':'Payment'}
...
def parse_fy(path, company, fy):
    ...
    for r in t2:
        ...
        # ---------- ITEM ROWS : bills, challans, orders ----------
        ...
        # ---------- LEDGER ROWS : payments and journals ----------
            ...
            out.append(dict(
                ...
                search_text=clean(f"{led} {grp} {r.get('ShortNar','')}").upper(),
            ))
    return out
