...
with call(where_at, fn, args) as (values
  ...
  ('busy.html:1005','busy_doc_types',array['p_company']::text[]),
  ('busy.html:1107','busy_search_count',array['p_company','p_query','p_party','p_doc_types','p_date_from','p_date_to']::text[]),
  ('busy.html:1116','busy_search',array['p_company','p_query','p_party','p_doc_types','p_date_from','p_date_to','p_limit','p_offset']::text[]),
  ('busy.html:1202','busy_monthly_totals',array['p_company']::text[]),
  ('busy.html:1289','busy_gst_ledger',array['p_company']::text[]),
  ('busy.html:1388','busy_row_counts',array[]::text[]),
  ('busy.html:1643','busy_import_abandon',array['p_batch_id']::text[]),
  ('busy.html:1662','busy_import_stage',array['p_batch_id','p_company','p_fy','p_rows']::text[]),
  ('busy.html:1681','busy_import_finalise',array['p_batch_id','p_company','p_fy']::text[]),
  ('busy.html:1817','busy_clear_preview',array[]::text[]),
  ('busy.html:1861','busy_clear_history',array['p_confirm']::text[]),
